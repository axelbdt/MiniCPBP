...
import static minicpbp.cp.Factory.abs;
import static minicpbp.cp.Factory.allDifferent;
import static minicpbp.cp.Factory.among;
import static minicpbp.cp.Factory.atleast;
import static minicpbp.cp.Factory.atmost;
import static minicpbp.cp.Factory.binPacking;
import static minicpbp.cp.Factory.cardinality;
import static minicpbp.cp.Factory.circuit;
import static minicpbp.cp.Factory.cumulative;
import static minicpbp.cp.Factory.disjunctive;
import static minicpbp.cp.Factory.element;
import static minicpbp.cp.Factory.equal;
import static minicpbp.cp.Factory.exactly;
import static minicpbp.cp.Factory.inverse;
import static minicpbp.cp.Factory.isEqual;
import static minicpbp.cp.Factory.isLarger;
import static minicpbp.cp.Factory.isLargerOrEqual;
import static minicpbp.cp.Factory.isLess;
import static minicpbp.cp.Factory.isLessOrEqual;
import static minicpbp.cp.Factory.isNotEqual;
import static minicpbp.cp.Factory.larger;
import static minicpbp.cp.Factory.largerOrEqual;
import static minicpbp.cp.Factory.less;
import static minicpbp.cp.Factory.lessOrEqual;
import static minicpbp.cp.Factory.lexLess;
import static minicpbp.cp.Factory.lexLessOrEqual;
import static minicpbp.cp.Factory.makeBoolVar;
import static minicpbp.cp.Factory.makeDfs;
import static minicpbp.cp.Factory.makeIntVar;
import static minicpbp.cp.Factory.makeLds;
import static minicpbp.cp.Factory.makeSolver;
import static minicpbp.cp.Factory.maximum;
import static minicpbp.cp.Factory.minimum;
import static minicpbp.cp.Factory.minus;
import static minicpbp.cp.Factory.modulo;
import static minicpbp.cp.Factory.mul;
import static minicpbp.cp.Factory.nValues;
import static minicpbp.cp.Factory.negTable;
import static minicpbp.cp.Factory.not;
import static minicpbp.cp.Factory.notEqual;
import static minicpbp.cp.Factory.or;
import static minicpbp.cp.Factory.plus;
import static minicpbp.cp.Factory.pow;
import static minicpbp.cp.Factory.product;
import static minicpbp.cp.Factory.quotient;
import static minicpbp.cp.Factory.regular;
import static minicpbp.cp.Factory.shortTable;
import static minicpbp.cp.Factory.sum;
import static minicpbp.cp.Factory.sumModP;
...
public class XCSP implements XCallbacks2 {
	...
	public XCSP(String fileName) throws Exception {
		...
		minicp.setTraceBPFlag(true);
		minicp.vanillaBP(10);
		minicp.setTraceBPFlag(false);
	}
	...
	/*
	 * eventually useful? private IntVar[][] mapVar2dArray(Object vars) { return
	 * Arrays.stream((XVarInteger[][])
	 * vars).map(this::mapVarArray).toArray(IntVar[][]::new); }
	 */
	 ...
	 * TODO
	 * 
	 * @Override public void buildCtrPrimitive(String id, XVarInteger x,
	 * Types.TypeConditionOperatorRel op, int[] t) { if (hasFailed) return;
	 * 
	 * }
	 */
	 ...
	 * 
	 * @Override public void buildCtrPrimitive(String id, XVarInteger x,
	 * TypeConditionOperatorSet op, int min, int max) { if (hasFailed) return;
	 * 
	 * }
	 */
	...
	public void buildCtrLogic(String id, XVarInteger x, XVarInteger y, Types.TypeConditionOperatorRel op,
			XVarInteger z) {
		if (hasFailed)
			return;
		try {
	...
	/*
	 * TODO public void buildCtrLogic(String id, TypeLogicalOperator lop,
	 * XVarInteger[] vars) public void buildCtrLogic(String id, XVarInteger x,
	 * TypeEqNeOperator op, TypeLogicalOperator lop, XVarInteger[] vars)
	 */
	...
	private IntVar arithmeticOperatorConstraintVal(IntVar x, Types.TypeArithmeticOperator aop, int p) {
		...
		case MUL:
			return mul(x, p);
		case DIV:
			IntVar y = makeIntVar(minicp, (p >= 0 ? (int) Math.floor(x.min() / p) : (int) Math.floor(x.max() / p)),
					(p >= 0 ? (int) Math.ceil(x.max() / p) : (int) Math.ceil(x.min() / p)));
...
//						System.out.println("in rel with val; type is " + type);
					return reifiedRelOperatorConstraintVal(parseExpr(tree.sons[0]), type.toRelop(),
							tree.sons[1].val(0));
				} else {
					return reifiedRelOperatorConstraintVar(parseExpr(tree.sons[0]), type.toRelop(),
							parseExpr(tree.sons[1]));
				}
			} else if (type.isArithmeticOperator()) {
				if (tree.sons[1].type == Types.TypeExpr.LONG) {
//						System.out.println("in arith with val; type is " + type);
					return arithmeticOperatorConstraintVal(parseExpr(tree.sons[0]), type.toAriop(),
							tree.sons[1].val(0));
				} else {
					return arithmeticOperatorConstraintVar(parseExpr(tree.sons[0]), type.toAriop(),
							parseExpr(tree.sons[1]));
				}
	...
	public void buildCtrSum(String id, XVarInteger[] list, Condition condition) {
		...
		try {
			if ((condition instanceof Condition.ConditionVar)
				...
				// special case of equality with a variable: avoid creating an intermediate
				// variable
	...
	// TODO: public void buildCtrOrdered(String id, XVarInteger[] list,
	// XVarInteger[] lengths, TypeOperatorRel operator)
	...
	public void buildCtrPrecedence(String id, XVarInteger[] list, int[] values, boolean covered) {
		...
		try {
			...
			// TODO: potentially wasteful in terms of space; define A over union of domains
			// only
			for (int i = 0; i <= values.length; i++) {
				...
				if (i < values.length)
					A[i][values[i]] = i + 1; // first occurrence of values[i]
	...
	@Override
	public void buildCtrElement(String id, XVarInteger[] list, int startIndex, XVarInteger index, Types.TypeRank rank,
			Condition cond) {
		if (hasFailed)
			return;
		try {
	...
	@Override
	public void buildCtrElement(String id, int[] list, int startIndex, XVarInteger index, Types.TypeRank rank,
			Condition cond) {
		if (hasFailed)
			return;
		try {
	...
	@Override
	public void buildCtrElement(String id, int[][] matrix, int startRowIndex, XVarInteger rowIndex, int startColIndex,
			XVarInteger colIndex, Condition cond) {
		if (hasFailed)
			return;
		try {
	 ...
	 * TODO?
	 * 
	 * @Override public void buildCtrCount(String id, XVarInteger[] list,
	 * XVarInteger[] values, Condition condition) { }
	 */
	@Override
	 ...
	 * TODO?
	 * 
	 * @Override public void buildCtrNValuesExcept(String id, XVarInteger[] list,
	 * int[] except, Condition condition) { }
	 */
	...
	public void buildCtrRegular(String id, XVarInteger[] list, Transition[] transitions, String startState,
		...
		try {
			...
			// create var view of x with domain starting at zero (regular constraint
	...
	// TODO: buildCtrStretch(String id, XVarInteger[] list, int[] values, int[]
	// widthsMin, int[] widthsMax);
	// TODO: buildCtrStretch(String id, XVarInteger[] list, int[] values, int[]
	// widthsMin, int[] widthsMax, int[][] patterns);

	// TODO: buildCtrMDD(String id, XVarInteger[] list, Transition[] transitions)
	// http://xcsp.org/format3.pdf

	@Override
	public void buildCtrCircuit(String id, XVarInteger[] list, int startIndex) {
		// TODO this signature (without size parameter) should allow self-loops but it
		// currently won't...
		if (hasFailed)
			return;
		try {
	...
	public void buildCtrBinPacking(String id, XVarInteger[] list, int[] sizes, int[] capacities, boolean loads) {
		// If loads is true then these capacities correspond to the exact load.
		// Difference between <limits> and <loads>
		if (hasFailed)
			return;
		try {
			IntVar[] b = mapVarArray(list);
	...
	@Override
	public void buildCtrBinPacking(String id, XVarInteger[] list, int[] sizes, XVarInteger[] capacities,
			boolean loads) {
		// If loads is true than these capacities correspond to the exact load.
		// Difference between <limits> and <loads>
		if (hasFailed)
			return;
		try {
	...
	@Override
	public void buildCtrKnapsack(String id, XVarInteger[] list, int[] weights, Condition wcondition, int[] profits,
		...
		try {
			...
			if ((woperator != Types.TypeConditionOperatorRel.EQ) && (woperator != Types.TypeConditionOperatorRel.LE)
					&& (woperator != Types.TypeConditionOperatorRel.LT)) {
				System.out.println("c Knapsack constraint with an unsupported wcondition");
				System.exit(1);
			}
			...
			if ((poperator != Types.TypeConditionOperatorRel.EQ) && (poperator != Types.TypeConditionOperatorRel.GE)
					&& (poperator != Types.TypeConditionOperatorRel.GT)) {
				System.out.println("c Knapsack constraint with an unsupported pcondition");
				System.exit(1);
			}
	...
	public void buildCtrCumulative(String id, XVarInteger[] origins, int[] lengths, int[] heights,
		...
		try {
			IntVar[] starts = mapVarArray(origins);
			if (!(condition instanceof Condition.ConditionVal)
					|| (((Condition.ConditionVal) condition).operator != Types.TypeConditionOperatorRel.LE)) {
...
//		minicp.setVariationThreshold(variationThreshold);
		...
		/*
		 * Stream<IntVar> nonDecisionVars = mapVar.entrySet().stream().sorted(new
		 * EntryComparator()) .map(Map.Entry::getValue).filter(v ->
		 * !decisionVars.contains(v)); IntVar[] vars =
		 * Stream.concat(decisionVars.stream(), nonDecisionVars).toArray(IntVar[]::new);
		 */
		...
		/*
		 * // GP for branching, use all vars registered in solver, not only those
		 * appearing in the model IntVar[] vars = new
		 * IntVar[minicp.getVariables().size()]; for (int i = 0; i <
		 * minicp.getVariables().size(); i++) { vars[i] = minicp.getVariables().get(i);
		 * }
		 */
