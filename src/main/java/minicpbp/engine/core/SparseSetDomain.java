...
public class SparseSetDomain implements IntDomain {
	...
	public SparseSetDomain(Solver cp, int min, int max) {
		...
		tieBreakers = new double[max - min + 1];
		for (int i = 0; i < tieBreakers.length; i++) {
			tieBreakers[i] = rand.nextDouble() / (10 * size());
		}
		System.out.print("");
	}
	...
	public void resetMarginals() {
		int s = fillArray(domainValues);
		for (int j = 0; j < s; j++) {
			setMarginal(domainValues[j], beliefRep.subtract(beliefRep.one(), beliefRep.std2rep(tieBreakers[j])));
		}
	}
	...
	@Override
	public double tieBreaker(int value) {
		return tieBreakers[value];
	}
